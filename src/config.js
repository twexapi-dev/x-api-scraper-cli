...
export function resolveAppState(state, config) {
  ...
  return {
    ...
    apiKey: state.apiKey || process.env.X_API_SCRAPER_KEY || configuredApp?.apiKey || "",
    baseUrl: (
      state.baseUrl ||
      process.env.X_API_SCRAPER_BASE_URL ||
...
export async function handleAuthAppsCommand(state, config, args) {
  ...
  if (action === "add") {
    ...
    if (!name || !apiKey) {
      exitWithError("Usage: x-api-scraper auth apps add --name <name> --api-key <key> [--base-url <url>]");
    }
  ...
  if (action === "use") {
    const name = args[3];
    if (!name) {
      exitWithError("Usage: x-api-scraper auth apps use <name>");
    }
    if (!config.apps[name]) {
  ...
  if (action === "remove") {
    const name = args[3];
    if (!name) {
      exitWithError("Usage: x-api-scraper auth apps remove <name>");
    }
    if (!config.apps[name]) {
      exitWithError(`Unknown app profile: ${name}`);
    }
...
export async function handleAuthProfilesCommand(state, config, args) {
  ...
  if (action === "add") {
    ...
    if (!name || (!cookie && !authToken)) {
      exitWithError("Usage: x-api-scraper auth profiles add --name <name> [--cookie <value> | --auth-token <value>] [--ct0 <value>]");
    }
  ...
  if (action === "use") {
    const name = args[3];
    if (!name) {
      exitWithError("Usage: x-api-scraper auth profiles use <name>");
    }
  ...
  if (action === "remove") {
    const name = args[3];
    if (!name) {
      exitWithError("Usage: x-api-scraper auth profiles remove <name>");
    }
    if (!config.profiles[name]) {
      exitWithError(`Unknown profile: ${name}`);
    }
